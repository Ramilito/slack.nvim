...
fn resource() -> Resource {
    Resource::builder()
        .with_schema_url(
            [
                KeyValue::new(SERVICE_NAME, "slack.nvim"),
                KeyValue::new(SERVICE_VERSION, "2.0.0"),
            ],
            SCHEMA_URL,
        )
        .with_service_name("slack.nvim")
        .build()
}
...
/// Public entry point called by slack-core.
pub fn setup_logger(
    log_file_path: &str,
    endpoint: &str,
    ...
    let (file_layer, guard) = {
        let file = File::create(format!("{}/slack.log", log_file_path))?;
    ...
    // ---- 3. OTLP layer ----
    let otel_layer = OpenTelemetryLayer::new(provider.tracer("slack.nvim"));
